...
class Gateway extends Worker
{
    ...
    /**
     * blog 内部通讯起始端口，每个 blog 实例应该都不同，步长1000
     *
     * @var int
     */
    ...
    /**
     * 是否可以平滑重启，blog 不能平滑重启，否则会导致连接断开
     *
     * @var bool
     */
    ...
    protected function sendToWorker($cmd, $connection, $body = '')
    {
        ...
        if ($this->_workerConnections) {
            ...
            // blog 启动后 1-2 秒内 SendBufferToWorker fail 是正常现象，因为与 worker 的连接还没建立起来，
            // 所以不记录日志，只是关闭连接
            $time_diff = 2;
    ...
    public function onWorkerStart()
    {
        ...
        // 初始化 blog 内部的监听，用于监听 worker 的连接已经连接上发来的数据
        ...
        // 注册 blog 的内部通讯地址，worker 去连这个地址，以便 blog 与 worker 之间建立起 TCP 长连接
    ...
    public function onWorkerMessage($connection, $data)
    {
		...
		$connection->key = $key;
            ...
            case GatewayProtocol::CMD_GET_CLIENT_ID_BY_UID:
                ...
                return;
            default :
                $err_msg = "blog inner pack err cmd=$cmd";
